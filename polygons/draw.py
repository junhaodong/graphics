...
# points entered counter-clockwise
...
def draw_polygons( points, screen, color ):
    ...
    for i in range(len(points))[::3]:
        a = [points[i+1][j]-points[i][j] for j in range(3)]
        b = [points[i+2][j]-points[i][j] for j in range(3)]
        normal = crossProd(a,b)
        if dotProd(normal, viewVec) > 0:
